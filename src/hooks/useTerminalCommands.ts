...
type TerminalState = 'initial' | 'menu' | 'framework-selection' | 'project-name' | 'css-library' | 'css-library-selection' | 'installing';
...
4. Start the development server: npm run dev ·`;
      ...
      case 'css-library-selection':
        if (['1', '2'].includes(command)) {
          ...
          setCurrentState('initial');
          const additionalSteps = selectedCssLibrary === 'shadcn' 
            ? `\n5. Initialize Shadcn/ui: npx shadcn${selectedFramework === 'react' ? '' : '-vue'}@latest init` 
            : '';
          return `${result}\n\nNext steps:
1. Extract the downloaded zip file ·
2. Navigate to the project directory: cd ${projectName} ·
